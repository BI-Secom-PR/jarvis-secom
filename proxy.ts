...
function buildCsp(nonce: string): string {
  ...
  return [
    ...
    // Creative thumbnails need no host of their own: they live in an IDrive e2
    // bucket that cannot be public on the free tier, so /api/sentimentos/thumb
    // presigns and streams them same-origin ('self'). The CDN hosts below stay
    // only for the rows that still carry a raw Meta/TikTok image_url.

"""
Parser ADMOTION — comprovante de entrega + verification de URLs.

Comprovante (formato DCM/Campaign Manager 360):
  Sheet "Data" (primeira sheet). Cabeçalho com:
    "Site (CM360)" = veículo
    "Impressions"  = entregue
    "Active View: Viewable Impressions" = viewables
    "Active View: % Viewable Impressions" = viewability (decimal → %)
    "Flight Booked Units" = contratado
    "Clicks" = cliques
  Filtrar linhas com nome de veículo "---" e subtotais " Total".

Verification:
  Sheet "VERIFICATION" (wb.active). Colunas:
    "Veículos" (plural), "URL Veiculada", "Impressões Totais", "Categoria".
  Header detection: 'categoria' + qualquer coluna contendo 'url'.
"""
...
def _normalize_va(v) -> float | None:
    """Decimal (0.80) → percentagem (80.0). Mantém se já > 1."""
    f = to_float(v)
    if f is None:
        return None
...
def _find_comp_header(ws):
    """
    Varre até 25 linhas buscando cabeçalho com 'Site (CM360)' OU 'Veículo'
    E ('Impressions' OU 'Impressões' OU 'Entregues').
    """
...
def parse_comprovante(
    ...
    """Parseia comprovante ADMOTION (formato CM360 / DCM)."""
    path = Path(filepath)
    if not path.exists():
    ...
    return [
        {
            ...
            "indevidas":         {},
            "url_sample":        [],
            "formato_detectado": "admotion_comprovante",
        }
...
def parse_verif(
    ...
    """Parseia verification ADMOTION (URL Veiculada, Veículos, Impressões Totais)."""
    path = Path(filepath)
    if not path.exists():
        raise FileNotFoundError(f"Arquivo não encontrado: {filepath}")
    ...
    if i_veiculo is None or i_impressoes is None or i_categoria is None:
        wb.close()
        raise ValueError(
            f"Colunas obrigatórias ausentes (Veículos/Impressões Totais/Categoria): {path.name}"
        )
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Parser ADMOTION")

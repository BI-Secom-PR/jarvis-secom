# Graph Report - jarvis_ui  (2026-06-09)

## Corpus Check
- 93 files · ~48,094 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 960 nodes · 1361 edges · 64 communities (52 shown, 12 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 95 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `919d83a8`
...
7. `to_int()` - 13 edges
8. `Framework Analítico — Performance de Conteúdos do Governo Federal` - 13 edges
9. `Status Diário Nov 5 2026 - Posicionamento SP (Novo Posicionamento Estadual SP / CALIA)` - 12 edges
10. `normaliza_categoria()` - 11 edges
...
- `GET()` --calls--> `requireAuth()`  [INFERRED]
  app/api/exports/[id]/route.ts → lib/auth.ts
...
Cohesion: 0.13
Nodes (15): AdminLayout(), AdminPage(), metadata, PATCH(), POST(), schema, db, sessions (+7 more)

### Community 1 - "API Routes & SQL Engine"
Cohesion: 0.15
Nodes (11): COLORS, Props, STROKE_COLORS, ChartWidget, MessageBubble(), Props, renderMarkdown(), ChartData (+3 more)
...
Cohesion: 0.05
Nodes (60): CHART_SPEC_SCHEMA, CreateDownloadArgs, CreateDownloadCtx, createDownloadFile(), executeSql(), google, ollamaClient, OllamaMessage (+52 more)

### Community 5 - "User Data Model"
Cohesion: 0.05
Nodes (43): email, enabled, name, password_hash, role, name, notNull, primaryKey (+35 more)

### Community 6 - "AI Chat & KPI Framework"
Cohesion: 0.09
Nodes (32): AI Chat Pipeline (NL → SQL → MySQL → Chart), CHART_REQUEST Sentinel Pattern for Inline Chart Rendering, Dual-Database Pattern (PostgreSQL auth + MySQL analytics), Gold DB Column Mapping for KPI SQL Formulas, gold_platforms_campaigns — Main Analytics Fact Table (MySQL), index.html — Standalone n8n Webhook Chat UI (visionOS Design), KPI Formula Definitions (CPV, VTR, VTRc, CTR, CPM, CPC, CPE, CPA), Meta Ads Visualizações KPIs — ThruPlay, CPV, TPR, VTRc (PDF benchmarks) (+24 more)

### Community 7 - "DB Schema Relations"
Cohesion: 0.04
Nodes (48): name, notNull, primaryKey, type, columnsFrom, columnsTo, name, onDelete (+40 more)

### Community 8 - "Authentication Flow"
Cohesion: 0.08
Nodes (21): POST(), getSession, LoginPage(), metadata, GET(), metadata, RegisterPage(), buildEngineResponse() (+13 more)

### Community 9 - "Frontend Dependencies"
Cohesion: 0.17
Nodes (12): devDependencies, @anthropic-ai/claude-code, drizzle-kit, tailwindcss, @tailwindcss/postcss, @types/dom-speech-recognition, @types/node, @types/pdfkit (+4 more)

### Community 10 - "Home & Navigation UI"
Cohesion: 0.09
Nodes (14): PALETTE, Particle, ADSERVERS, CRITERIO_OPTIONS, ESTADOS_BRASIL, formatElapsed(), MONTHS, STATUS_BG (+6 more)
...
Cohesion: 0.07
Nodes (30): 1\. Estrutura dos Dados, 2.1 Eixo Temático — "Sobre o que a peça fala", 2.2 Programa / Entrega — "Qual programa governamental a peça divulga", 2.3 Formato de Mídia — "Que tipo de peça é", 2.4 Segundagem — "Quanto tempo dura a peça", 2.5 Elemento Visual Principal — "O que a audiência vê", 2.6 Tom da Mensagem — "Como a peça fala com a audiência", 2.7 Porta-Voz — "Quem representa a mensagem" (+22 more)

### Community 18 - "Users Admin Table"
Cohesion: 0.19
Nodes (10): EditState, UserRow, User, SessionUser, logout(), createSession(), saveMessage(), updateSessionTitle() (+2 more)
...
Cohesion: 0.12
Nodes (16): GET(), PATCH(), Home(), metadata, GET(), POST(), MenuCardProps, chatMessages (+8 more)
...
Cohesion: 0.12
Nodes (16): 1. Prerequisites, 2. Environment variables, 3. Database setup, 4. Run, Admin panel features, Authentication, Chat Features, Database Scripts (+8 more)
...
Cohesion: 0.13
Nodes (6): ChatPage(), metadata, VoiceMode, InputArea, InputAreaHandle, Props

### Community 37 - "Chat Message Schema"
Cohesion: 0.13
Nodes (14): 1. Fórmulas Oficiais SECOM, 2. Médias da SECOM por Plataforma e Objetivo, 3. Mapeamento de Colunas no Gold DB, 4. Notas Gerais, Amazon DSP (BRL — custos já convertidos no gold DB), Disponibilidade por plataforma (gold_platforms_campaigns):, Google Ads (BRL), Kwai Ads (BRL) (+6 more)

### Community 38 - "Filename Schema"
Cohesion: 0.20
Nodes (9): GET(), bytea, ChatMessage, ChatSession, FileExport, fileExports, messageRoleEnum, roleEnum (+1 more)

### Community 39 - "File Size Schema"
Cohesion: 0.33
Nodes (6): cleanEnv(), isNeonHost, parsePort(), pgEnv, requiredEnv(), globalForPg
...
Cohesion: 0.18
Nodes (11): scripts, build, db:generate, db:migrate, db:push, db:seed, dev, start (+3 more)
...
Cohesion: 0.29
Nodes (7): overrides, esbuild, postcss, protobufjs, svix, tmp, ws
...
## Knowledge Gaps
- **418 isolated node(s):** `date`, `metadata`, `metadata`, `metadata`, `UserRow` (+413 more)
...
- **Why does `tables` connect `DB Schema Relations` to `Community 58`, `DB Schema Columns`, `User Data Model`, `DB Migration Schema`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `columns` connect `File Upload Schema` to `DB Primary Keys`, `DB Schema Columns`, `Community 61`, `DB Timestamps`?**
...
- **Should `Campaign Reports & Agencies` be split into smaller, more focused modules?**
